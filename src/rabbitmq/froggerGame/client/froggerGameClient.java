...
import rabbitmq.froggerGame.server.froggerGameFactoryRI;
import rabbitmq.froggerGame.server.froggerGameSessionRI;
import rabbitmq.util.rmisetup.SetupContextRMI;
...
public class froggerGameClient {
    ...
    public void menuGame(froggerGameSessionRI froggerGameSessionRI) throws IOException {
        ...
        switch (op.nextInt()) {
            case 1:
                ...
                game1 = froggerGameSessionRI.createGame(lowerCaseDifficulty);

                while(game1.getNplayers() < 2){
                    System.out.println("Nplayers = " + game1.getNplayers());
                }

                pb = new ProcessBuilder("D:\\Programacao\\IntelliJ\\IntelliJProjects\\ProjetoSD\\src\\rabbitmq\\froggerGame\\runscripts\\runobserber.bat " + (game1.getNplayers() - 1) + " " + game1.getId() + " " + dificuldade);
                p = pb.start();

                froggerGameSessionRI.exitGame(game1.getId());
                menuGame(froggerGameSessionRI);
                break;
            case 2:
                ...
                game1 = froggerGameSessionRI.joinGame(game.nextInt());
                dificuldade = transformDifficulty(game1.getDifficulty());
                if (dificuldade == 0) return;

                pb = new ProcessBuilder("D:\\Programacao\\IntelliJ\\IntelliJProjects\\ProjetoSD\\src\\rabbitmq\\froggerGame\\runscripts\\runobserver.bat " + (game1.getNplayers() - 1) + " " + game1.getId() + " " + dificuldade);
                p = pb.start();
